...
local function ensure_pane()
  ...
  -- Verify the stored pane still exists by checking if we can find it
  local found_pane = find_claude_pane()
  if found_pane == claude_pane then
    return true
  else
...
local function start_claude(split_flag)
  ...
  if pane and #pane > 0 then
    ...
    -- Block execution until Claude finishes initializing to prevent send commands from failing
    -- This ensures Claude is ready before ClaudeSendFile/ClaudeSendBuffer can be used
    os.execute("sleep 1")
  else
