...
local function find_claude_panes()
  local panes_output = sh("tmux list-panes -a -F '#{pane_id}|#{session_name}|#{window_name}|#{window_index}|#{pane_index}|#{pane_current_command}'")
  if not panes_output then return {} end

  local panes = {}
  for line in panes_output:gmatch("[^\n]+") do
    local pane_id, session, window, win_idx, pane_idx, command = line:match("^(%%[0-9]+)|([^|]*)|([^|]*)|([^|]*)|([^|]*)|(.+)$")
...
local function wait_for_claude_ready(pane_id, callback, timeout_ms)
  ...
  local function check()
    vim.schedule(function()
      ...
      -- Look for Claude Code prompt indicators (works with both default and vim mode)
      if output and output ~= "" and (
        output:match("Claude Code") or      -- Banner text (any version)
        output:match(">%s") or              -- Prompt ready for input (most reliable indicator)
        output:match("Try \"") or           -- Suggestion line
        output:match("INSERT") or           -- Vim mode indicator
        output:match("NORMAL") or           -- Vim normal mode
        string.find(output, "───", 1, true)  -- Separator line (plain search)
      ) then
        if callback then callback(true) end
      else
...
local function format_pane_desc(pane)
  return string.format("%s:%s [window %s, pane %s] (%s)",
    pane.session, pane.window, pane.window_index, pane.pane_index, pane.id)
end
...
local function send_visual_range()
  ...
  if not range_ref then return end
